...
该接口提供现货交易中已完成的大额限价挂单的历史数据。(阈值: BTC ≥ 35万, ETH ≥ 25万, Other ≥ 1万)
...
| start_time | string | YES | 开始时间戳（毫秒）（例如：1641522717000）。                                 |
| end_time   | string | YES | 结束时间戳（毫秒）（例如：1641522717000）。                                 |
| state      | string | YES | 订单状态说明如下：  1：进行中（In Progress）  2：已完成（Finish）  3：已撤销（Revoke）  |
