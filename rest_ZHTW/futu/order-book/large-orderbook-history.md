...
| start_time | integer | YES | 起始時間戳，單位為毫秒（例如：1746776387000）。                               |
| end_time   | integer | YES | 結束時間戳，單位為毫秒（例如：1746776587000）。                               |
| state      | integer | YES | 訂單狀態說明如下：1：進行中（In Progress） 2：已完成（Finish） 3：已撤銷（Revoke）      |
